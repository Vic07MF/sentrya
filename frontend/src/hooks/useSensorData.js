...
export function useSensorData() {
  ...
  // ------------------------------------------------------------------
  // Dados iniciais: SQLite (leituras + anomalias Isolation Forest)
  // SEM getEventos — event_permanent removido para testes
  // ------------------------------------------------------------------
  const fetchInitial = useCallback(async () => {
    try {
      ...
      if (histRes.status === "ok" && Array.isArray(histRes.data)) {
        ...
        // Inicializa data com a última leitura
        if (pts.length > 0) {
