...
import { ALERT_LIMIT, CRITICAL_LIMIT, getVibrationStatus } from "@/lib/utils";
  ...
  useEffect(() => {
    const fetchMachines = async () => {
      try {
        ...
        if (!response.ok) throw new Error("Falha ao buscar máquinas");
    ...
    <div className="space-y-4">
      <div className="mb-6">
        ...
        <p className="mt-1 text-xs font-mono text-brand-muted">
          {machines.length} máquina{machines.length !== 1 ? "s" : ""} ativa
          {machines.length !== 1 ? "s" : ""}
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {machines.map((machine) => (
          <Card
            ...
            className="p-5"
            variant={machine.status === "CRITICO" ? "danger" : "default"}
          >
            <div className="mb-4 flex items-start justify-between">
              <div>
                ...
                <p className="mt-1 font-mono text-[11px] text-brand-muted">
                  Vibração: {(machine.vib_rms * 100).toFixed(2)} mm/s
                </p>
              </div>
              <Badge
                variant={
                  machine.status === "CRITICO"
                    ? "danger"
            ...
            <div className="space-y-3 border-t border-brand-border pt-4">
              ...
              <div className="mt-3 h-1 overflow-hidden rounded-full bg-white/5">
                <div
                  ...
                  style={{
                    width: `${Math.min(
                      ((machine.vib_rms * 100) / CRITICAL_LIMIT) * 100,
                      100,
                    )}%`,
                    background:
                      machine.vib_rms * 100 > CRITICAL_LIMIT
                        ? "#e55353"
                        : machine.vib_rms * 100 > ALERT_LIMIT
                          ? "#f59f0f"
                          : "#3bc9a0",
                  }}
                />
            ...
            <div className="mt-3 border-t border-brand-border pt-3 text-right">
              <p className="text-[10px] font-mono text-brand-muted">
                {new Date(machine.timestamp).toLocaleTimeString("pt-BR")}
              </p>
            </div>
          </Card>
        ))}
