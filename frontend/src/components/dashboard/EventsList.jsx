...
function EventItem({ event, faded = false }) {
  ...
  return (
    <div
      className={`relative flex items-start gap-3 rounded-lg border px-3 py-2.5 ${s.bg} ${s.border} ${faded ? "overflow-hidden" : ""}`}
    >
    ...
    <Card className="flex-1 p-4">
      ...
      {recent.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {visible.map((ev, idx) => {
            // O último card visível recebe degradê APENAS quando há mais e não está expandido
